...
interface Service {
  ...
  status: string;
  created_at: string;
  cover_asset?: { public_url: string }; // joined
}
...
export const AdminServices: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Service>>({
    title: '', slug: '', short_description: '', description: '', category: 'Pemasangan',
    cover_asset_id: null, gallery_assets: [], features: [], technical_details: '', applications: [],
  ...
  const categories = ['Pemasangan', 'Pembuatan', 'Service / Maintenance', 'Konsultasi', 'Lainnya'];
  ...
  const handleEdit = (service: Service) => {
    setFormData({
      ...
      gallery_assets: service.gallery_assets || [],
    ...
    return (
      <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '4rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          >
            <Save size={18} /> {formSaving ? 'Saving...' : 'Save Service'}
          </button>
        </div>
        ...
        <form onSubmit={saveForm} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <section style={{ backgroundColor: '#FFFFFF', padding: '2rem', borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.5rem', color: '#334155' }}>Service Title *</label>
              ...
              <div>
                ...
                <textarea value={formData.short_description || ''} onChange={e => setFormData({...formData, short_description: e.target.value})} style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #CBD5E1', boxSizing: 'border-box', minHeight: '80px', fontFamily: 'inherit' }} placeholder="Brief summary for cards and lists..." />
              </div>
            </div>
          </section>
          ...
          <section style={{ backgroundColor: '#FFFFFF', padding: '2rem', borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, margin: '0 0 1.5rem 0', color: '#0F172A', borderBottom: '1px solid #F1F5F9', paddingBottom: '0.75rem' }}>Service Details</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.5rem', color: '#334155' }}>Technical Details</label>
                <textarea 
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
            ...
            <button type="submit" disabled={formSaving} style={{ padding: '0.75rem 1.5rem', borderRadius: '8px', border: 'none', backgroundColor: '#0EA5E9', color: '#FFFFFF', fontWeight: 600, cursor: formSaving ? 'not-allowed' : 'pointer' }}>
              {formSaving ? 'Saving...' : 'Save Service'}
            </button>
          </div>
        </form>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800, margin: '0 0 0.25rem 0', fontFamily: 'Manrope' }}>Services</h2>
          <p style={{ color: '#64748B', margin: 0, fontSize: '0.9rem' }}>Manage services offered by BQA.</p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ backgroundColor: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
              <th style={{ padding: '1rem 1.5rem', fontWeight: 600, color: '#334155', fontSize: '0.85rem' }}>Service</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} style={{ padding: '3rem', textAlign: 'center', color: '#64748B' }}>Loading services...</td></tr>
            ) : filteredServices.length === 0 ? (
              <tr><td colSpan={5} style={{ padding: '3rem', textAlign: 'center', color: '#64748B' }}>No services found.</td></tr>
            ) : (
                  ...
                  <td style={{ padding: '1rem 1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ width: '48px', height: '48px', borderRadius: '8px', backgroundColor: '#F1F5F9', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                         {s.cover_asset?.public_url ? (
                           <img src={s.cover_asset.public_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                         ) : (
                           <ImageIcon size={20} color="#CBD5E1" />
                         )}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, color: '#0F172A', marginBottom: '0.1rem' }}>{s.title}</div>
                        <div style={{ fontSize: '0.75rem', color: '#64748B' }}>/{s.slug}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '1rem', color: '#475569', fontSize: '0.9rem' }}>{s.category}</td>
                  <td style={{ padding: '1rem', color: '#475569', fontSize: '0.9rem' }}>{s.sort_order}</td>
                  <td style={{ padding: '1rem' }}>
                    <span style={{ 
                      padding: '0.25rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600, 
                      backgroundColor: s.status === 'Published' && s.is_active ? '#DCFCE7' : '#FEF3C7', 
                      color: s.status === 'Published' && s.is_active ? '#166534' : '#92400E' 
                    }}>
                      {s.is_active ? s.status : 'Inactive'}
                    </span>
                  </td>
                  <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                    <button onClick={() => handleEdit(s)} style={{ background: 'none', border: 'none', color: '#64748B', cursor: 'pointer', marginRight: '0.5rem', padding: '0.5rem' }}><Edit3 size={18} /></button>
                    <button onClick={() => handleDelete(s.id, s.title)} style={{ background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', padding: '0.5rem' }}><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))
            )}
